...
var CIRCLES_PER_ROW = 8; // rows & columns
...
$(document).ready(function () {
            ...
            if (line && !gameOver) {
                ...
                if (e.target.getFill() == line.getStroke() && isDown) {
                    ...
                    // Lines can only be one unit long -- prevents line from streching across multiple dots
                     if (Math.abs(line.x2 - line.x1) > grid || Math.abs(line.y2 - line.y1) > grid) {
    ...
    function startTimer(defaultTime) {
        // Set the initial time
        $('.dyn-time').text(defaultTime);
        ...
        var timerInterval = setInterval(function () {
            ...
            if (timeLeft <= 0) {
                ...
                var currentScore = parseInt($('.dyn-score').text());
                var highScore = localStorage.getItem('highScore') || 0;
                
                if (currentScore > highScore) {
                    localStorage.setItem('highScore', currentScore);
                    $('.dyn-highscore').text(currentScore);
                    $('.dyn-alert').append(' New High Score!');
                }
    ...
    // Call the function with the desired default time
    startTimer(60); 
});
...
function updateScore(num_circles) {
    // Prevent score updates if the game is over
    if (gameOver) {
        return
    };

    var selection_score = Math.pow(2, num_circles); 
