document.addEventListener("DOMContentLoaded", () => {
    ...
    const resetButton = document.querySelector("#reset") // Add a reset button
    const width = 4
    let squares = []
    let score = 0
    ...
    function moveUp() {
        for (let i = 0; i < 4; i++) {
            let totalOne = squares[i].innerHTML || 0
            let totalTwo = squares[i + width].innerHTML || 0
            let totalThree = squares[i + width * 2].innerHTML || 0
            let totalFour = squares[i + width * 3].innerHTML || 0
    ...
    function addColours() {
        for (let i = 0; i < squares.length; i++) {
            if (squares[i].innerHTML == "") squares[i].style.backgroundColor = "#afa192"
    ...
    function resetGame() {
        ...
        // Re-enable key controls if disabled after a win or loss
        document.addEventListener("keydown", control);
    }
